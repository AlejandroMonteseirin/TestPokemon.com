...
def likers_protocol(self):
    if len(self.media_by_user) > 0:
        ...
        if self.media_by_user[self.current_index]["node"]["edge_liked_by"]["count"] >= 10 and self.media_by_user[self.current_index]["node"]["edge_liked_by"]["count"] < 100:
            get_user_id_post_page(self, self.media_by_user[self.current_index]["node"]['shortcode'])
