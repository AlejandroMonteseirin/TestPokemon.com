...
class InstaBot:
        ...
        if self.login_status:
            if self.login_status == 1:
                ...
                try:
                    ...
                    all_data = json.loads(r.text)
                    user_info = all_data['user']["username"]
    ...
    def auto_unfollow(self):
        ...
        if self.login_status:
            log_string = "Cogiendo Info : %s" % current_user
            self.write_log(log_string)
            if self.login_status == 1:
                ...
                try:
                    ...
                    i = 0
                    log_string = "Revisando la informacion de este usuario :3"
                    self.write_log(log_string)
                    ...
                    self.write_log(log_string)
                    log_string = "Fotos y eso : %i" % (media)
                    self.write_log(log_string)
                    if follows == 0 or follower / follows > 2:
                        self.is_selebgram = True
                        self.is_fake_account = False
                        print('   >>>Diria que es una celebridad y no nos va a seguir alex-sempai')
                    elif follower == 0 or follows / follower > 3:
                        self.is_fake_account = True
                        self.is_selebgram = False
                        print('   >>>Pinta a cuenta falsa sempai')
                    else:
                        self.is_selebgram = False
                        self.is_fake_account = False
                        print('   >>>Es una cuenta normal :3')
                    ...
                    if media > 0 and follows / media < 25 and follower / media < 25:
                        ...
                        print('   >>>Es bien uke (pasivo)')

                    if follow_viewer or has_requested_viewer:
                        self.is_follower = True
                        print("   >>>Nos sigue :3333")
                    else:
                        self.is_follower = False
                        print('   >>>NO nos sigue >:c')

                    if followed_by_viewer or requested_by_viewer:
                        self.is_following = True
                        print('   >>>nosotros le seguimos :3')

                    else:
                        self.is_following = False
                        print('   >>>nosotros NO le seguimos xq no nos acepto en su cuenta privada :c')
                ...
                return False
            print('               ****---juzgando individuo para ver si seguir siguiendole....')
            if ((self.is_selebgram is True or self.is_fake_account is True or self.is_active_user is False) and self.is_following is True and self.is_follower is False):
                    print('         ---*---*---*----UNFOLLOW DEL TIRI, Habernos seguido :(')
                    self.write_log(current_user)
                    self.unfollow(current_id)
                    insert_unfollow_count(self, user_id=current_id)
            else:
                    print('         -----*-*-*-*-----No te hacemos unfollow por wapo :3')
